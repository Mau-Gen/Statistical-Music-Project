...
--
/*!50003 DROP PROCEDURE IF EXISTS `CreateAUser` */;
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `CreateAUser`(uname VARCHAR(64), sub_type VARCHAR(64))
    DETERMINISTIC
BEGIN
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetMonthlyStats`(
    IN p_year INT,
    IN p_month INT
)
BEGIN 
    -- Result 1 - Top 5 songs
    SELECT s.song_name, COUNT(ld.listened_at) AS amount FROM listening_data ld
    JOIN songs s ON s.id = ld.song_id
    WHERE YEAR(ld.listened_at) = p_year
    AND 
    MONTH(ld.listened_at) = p_month
...
-- Dump completed on 2026-03-05 16:39:38
